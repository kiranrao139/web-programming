...
var url='mongodb://kiran:keer123@ds119343.mlab.com:19343/books';//1.Modify this url with the credentials of your db name and password.
...
app.get('/update/:toBeUpdated_id', function (req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        ...
        db.collection("insert").updateOne(myquery, newvalues,function(err, res) {
            if (err) throw err;
...
var insertDocument = function(db, data, callback) {
    db.collection('insert').insertOne( data, function(err, result) {
        if(err)
        {
...
var deleteDocument = function(db, data, callback) {
    db.collection('insert').deleteOne( data, function(err, result) {
        if(err)
        {
            res.write("Registration Failed, Error While Registering");
            res.end();
